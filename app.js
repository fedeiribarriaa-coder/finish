...
function renderDogs(){
  ...
  el.innerHTML=dogs.map(([n,meta,img])=>`
    <article class="dog-card">
      <img loading="lazy" src="${img}" alt="${n}, perro en adopción">
      <div class="pad">
        <h3>${n}</h3>
        <p>${meta}</p>
        <button class="mini-btn" data-dog="${n}">CONOCER →</button>
      </div>
    </article>
  `).join('');

  el.querySelectorAll('[data-dog]').forEach(
    b=>b.onclick=()=>openDog(b.dataset.dog)
  );
}

function openDog(name){
  const d=dogs.find(x=>x[0]===name);

  document.querySelector('#dogModalContent').innerHTML=`
    <img class="modal-dog" src="${d[2]}" alt="${name}">
    <span class="eyebrow">EN ADOPCIÓN</span>
    <h2>${name}</h2>
    <p><b>${d[1]}</b></p>
    <p class="story">
      ${stories[name]||'Está esperando una familia responsable que le dé una segunda oportunidad. Conocelo, completá la solicitud y el equipo de la ONG se comunicará con vos.'}
    </p>
    <div class="modal-actions">
      <button class="btn primary" onclick="openAdopt('${name}')">
        QUIERO ADOPTAR A ${name.toUpperCase()} ❤️
      </button>
    </div>
  `;
...
document
  .querySelector('#adoptForm')
  .addEventListener('submit',submitAdoption);
